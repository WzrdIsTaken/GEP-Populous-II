...
namespace
{
    std::unique_ptr<Engine::Driver> g_game;
}
...
int WINAPI wWinMain(_In_ HINSTANCE _hInstance, _In_opt_ HINSTANCE _hPrevInstance, _In_ LPWSTR _lpCmdLine, _In_ int _nCmdShow)
{
    ...
    g_game = std::make_unique<Engine::Driver>();
    ...
    // Register class and create window
    {
        ...
        SetWindowPos(hwnd, 0, 30, 30, 0, 0, SWP_NOSIZE | SWP_NOZORDER);
#else
        /// Hack around locations/auto scaling not being able to pull the actual window dimensions that instantiate the game
       ShowWindow(hwnd, _nCmdShow);
...
LRESULT CALLBACK WndProc(HWND _hWnd, UINT _message, WPARAM _wParam, LPARAM _lParam)
{
    ...
    auto game = reinterpret_cast<Engine::Driver*>(GetWindowLongPtr(_hWnd, GWLP_USERDATA));
    ...
    case WM_SYSKEYDOWN:
        if (_wParam == VK_RETURN && (_lParam & 0x60000000) == 0x20000000)
        {
            ...
            if (s_fullscreen)
            {
                ...
                int height = 600;
                if (game)
                    game->setDefaultSize(width, height);
